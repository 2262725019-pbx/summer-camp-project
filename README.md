...
GitHub 各分支原本包含多套不同的关键词检索器和重复知识。整合后只保留一套检索、评分、Top-K 和 Prompt 增强实现；知识库去重后包含 13 个主题：启动登录、API Key、本项目能力、天气语音图片、Tool/Skill/RAG 与消息路由、结果页网络、Git 密钥安全、对话记忆、Function Calling 与多工具协作、项目技术栈，以及来自学校官网的河南师范大学简介、校区和办学概况。开启和关闭对比测试可直接编辑本地配置：
