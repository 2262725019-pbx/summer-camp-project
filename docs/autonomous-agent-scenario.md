...
加入计划修复、进度消息、RAG 来源展示、待办同步和完整端到端演示脚本。当前单机版已使用 SQLite 完成跨进程断点、成品和提醒恢复；只有出现多实例部署需求时，才需要升级为共享数据库或外部工作流系统。

## 十六、8 月 26～27 日优化实现

- 目标解析增加常见口语表达，并自动识别增肌、减脂、提升体能、规律作息四种目标。
- 支持 3～14 天的中文或数字计划周期；输入数值越界时逐项指出错误字段，避免把无效资料误报为缺失。
- 天气成功结果缓存 10 分钟，RAG 成功结果缓存 30 分钟；缓存大小和有效期均可配置。同键并发请求合并为一次下游访问，不同键保持并行。
- RAG、天气、营养保持并行；后续训练、汇总、检查、页面与二维码继续按依赖串行。
- 身体资料、基础热量、蛋白质和七日安排由本地规则处理；只向 Skill 传递长度受限的 RAG、天气和健康相关历史。
- 图片封面改为可选且默认关闭，图片失败不阻塞主要成品。
- 必需步骤失败后将成功输出和步骤状态保存到本地 SQLite，用户在 30 分钟内发送“继续刚才的健康计划”即可跨程序重启从失败步骤恢复。
- 用户可发送“查看任务进度”查看完成比例和当前步骤，发送“取消健康计划”清除断点、成品和提醒。
- 完成计划后，用户可发送“开启每日健康提醒 07:30”订阅当天计划，发送“关闭健康提醒”取消；计划周期结束后自动取消订阅。完成计划和提醒订阅可跨程序重启恢复。
- SQLite 仅保存恢复所需的结构化资料，不保存原始消息、图片、语音或 API Key；数据库位于被 Git 忽略的 `runtime` 目录。
- 临时结果页增加禁止缓存、禁止页面嵌入、禁止来源泄露和内容安全策略响应头；页面仍仅用于可信局域网内的短期演示。
